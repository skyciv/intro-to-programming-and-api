 ...
 * @param {1 | 2 | 3} options.version The API version to use. Defaults to 3.
 * @param {"http" | "https"} options.http_or_https Whethere the request should use http or https. Defaults to https.
 */
let request = function (apiObject, callback, options = {}) {
	if (!options) options = {};
	...
	if (is_nodejs) {
		...
		var req = req_module.request(req_options, function (res) {
			...
			res.on('end', function () {
				...
				var parsedResponse = JSON.parse(res_data);
				callback(parsedResponse, options);
			});
		});
