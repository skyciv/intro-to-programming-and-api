...
# let's use the native python function 'range' to construct this list for us:
for i in range(5):
...
# What if we get a list that has more or fewer values?
# We can accommodate this using the native python function 'len' to get the number of items in the ist
N = len(f)

# Use the range keyword to construct an array of indexes to run the loop with:
for i in range(N):
	ftot += f[i] # sum the forces (remember our += operator from Lesson 1)
	mtot += f[i] * (mid - d[i]) # sum the products of the force & distance
